...
function GenerateReturnOne(const Query: TResolvedReturnQuery; const SqlName, ParamsName, ResultName: string; const ResultFields: array of string): string;
var
  CompleteGoParams, ScanFields, SqlVarName, PlaceholdersVarName: string;
...
begin
  ...
  begin
    ...
    Result := Result + '  row := q.DB.QueryRowContext(ctx, ' + SqlVarName + ', params...)' + #13#10;
    Result := Result + '  var i ' + ResultName + #13#10;
    Result := Result + '  err := row.Scan(' + #13#10;
    Result := Result + '    ' + ScanFields + #13#10;
    Result := Result + '  )' + #13#10;
    Result := Result + '  return &i, err' + #13#10;
  end
  else
  begin
